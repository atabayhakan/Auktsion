...
<template>
    ...
    <div class="w-full max-w-[1600px] mx-auto px-3 sm:px-5 lg:px-6 xl:px-8">
      ...
      <div class="flex items-center justify-between h-14 sm:h-16 lg:h-18 gap-2">
        ...
        <div class="flex items-center gap-2 sm:gap-3 shrink-0">
          <RouterLink to="/" class="flex items-center gap-2 group shrink-0" :aria-label="themeStore.theme.logoText || 'ITOrgo'">
            ...
            <img
              ...
              :style="{ height: (themeStore.theme.logoHeightPx || 36) + 'px' }"
            ...
            <template v-else>
              <div
                class="w-8 h-8 sm:w-9 sm:h-9 p-1 shadow-2xs group-hover:scale-105 transition-all flex items-center justify-center shrink-0 border border-amber-300/30 rounded-xl bg-gradient-to-br from-amber-400 via-amber-500 to-amber-600"
              >
                <IlbirsIcon class="w-full h-full text-gray-950" />
              </div>
              <div class="flex flex-col">
                <span class="font-sans font-black text-base sm:text-lg lg:text-xl text-gray-950 tracking-tight flex items-center leading-none">
                  {{ themeStore.theme.logoText || 'ITOrgo' }}<span class="w-1.5 h-1.5 rounded-full bg-amber-500 ml-0.5 shrink-0" />
                </span>
                <span class="text-[8px] text-gray-400 tracking-wider uppercase font-mono font-bold hidden sm:block mt-0.5 leading-none">
                  COMMERCE 2026
                </span>
              </div>
            </template>
          ...
          <!-- City Location Selector Dropdown (Always full name: Бишкек) -->
        ...
        <div class="flex items-center gap-1.5 sm:gap-2 shrink-0">
          ...
          <!-- Language Selector Dropdown (Both Desktop and Mobile) -->
          <div class="relative shrink-0">
            <Dropdown
            ...
            >
              <template #trigger>
                <button
                  type="button"
                  class="px-2 py-1.5 sm:px-2.5 sm:py-2 rounded-xl bg-slate-100 hover:bg-slate-200/80 border border-black/5 text-gray-800 transition-all text-xs font-bold flex items-center gap-1 cursor-pointer shadow-2xs shrink-0"
          ...
          <div class="relative shrink-0">
            ...
            >
              <template #trigger>
                <div class="flex items-center gap-1.5 px-2 py-1.5 sm:px-2.5 sm:py-2 rounded-xl bg-slate-100 hover:bg-slate-200/80 border border-black/5 transition-all cursor-pointer shadow-2xs">
            ...
            <div v-else class="hidden sm:flex items-center gap-1 sm:gap-1.5 shrink-0 whitespace-nowrap">
              <RouterLink
                to="/login"
      ...
      <!-- Row 2: Full-Width Mobile Dedicated Search Bar (< lg) -->
      <div class="lg:hidden pb-2.5 pt-0.5">
        <button
          type="button"
          class="w-full relative flex items-center bg-slate-100/90 hover:bg-slate-100 active:bg-white rounded-2xl border border-black/[0.08] px-3.5 py-2.5 transition-all duration-200 cursor-pointer text-left shadow-2xs group"
          @click="openSearch"
        >
          <Search class="w-4 h-4 text-gray-400 group-hover:text-amber-600 transition-colors shrink-0 mr-2.5" />
          <span class="text-xs font-medium text-gray-500 truncate flex-1">
            {{ t('nav.searchPlaceholder') || 'Что вы ищете? iPhone, Camry, Арашан...' }}
          </span>
          <span class="inline-flex items-center gap-1 text-[10px] font-mono px-2 py-0.5 rounded-lg bg-white border border-black/10 text-gray-400 shadow-2xs shrink-0">
            <Sparkles class="w-3 h-3 text-amber-500" />
            <span>ИИ поиск</span>
          </span>
        </button>
  ...
  <Transition name="sidebar">
    <div
      v-if="mobileMenuOpen"
      class="fixed inset-0 top-24 z-50 bg-black/40 backdrop-blur-sm lg:hidden"
      @click="closeMobileMenu"
    >
      <div class="h-[calc(100vh-6rem)] max-h-screen w-4/5 max-w-sm bg-white border-r border-black/10 p-5 sm:p-6 flex flex-col justify-between shadow-2xl overflow-y-auto custom-scrollbar" @click.stop>
        <div class="space-y-4">
          <RouterLink 
            to="/sell" 
